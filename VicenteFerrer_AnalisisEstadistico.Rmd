...
```{r}
testhipotesis <- function (m1, m2, NConf=0.95, var.equal=TRUE, alternative="two.sided") {
  mean1 <- mean(m1)
  n1 <- length(m1)
  sd1 <- sd(m1)
  mean2 <- mean(m2)
...
result<-testhipotesis(claim$UltCost, claim$IniCost, var.equal=FALSE, alternative = "two.sided")
result
```

## Conclusión

El valor crítico para un nivel de confianza del 95% es 1.959988 y el valor observado es 15.34648. Por lo tanto, nos encontramos en la zona de rechazo de la hipótesis nula y podemos concluir que hay diferencias entre IniCost y UltCost. Se concluye lo mismo con el valor p, que da un valor de 4.306053 × 10^−53^, muy inferior a alpha=0.05.

## Comprobación

```{r}
t.test( claim$UltCost, claim$IniCost, var.equal=FALSE, alternative = "two.sided")
```
